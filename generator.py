...
def convert_xlsx_to_xls(xlsx_path):
    soffice = shutil.which("soffice") or shutil.which("libreoffice")

    if not soffice:
        print("⚠ LibreOffice not found in PATH. Skipping XLS conversion.")
        return xlsx_path  # fallback to xlsx
    ...
    xls_path = os.path.splitext(xlsx_path)[0] + ".xls"
    return xls_path
...
def generate_sre(template_path, projects_path, owners_path, baseline_path, output_path):
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        for sheet_name, out_df in final_sheets.items():
            out_df.to_excel(writer, sheet_name=sheet_name, index=False)
    # Convert to XLS using LibreOffice
    xls_path = convert_xlsx_to_xls(output_path)

    # Return the XLS path so the web app can download it
    return xls_path
